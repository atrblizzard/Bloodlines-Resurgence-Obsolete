...
void CNB_Button::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	SetReleasedSound( "interface/mm_click_on.wav" );
}
...
void CNB_Button::PaintBackground()
{
	...
	DrawRoundedBox( 0, 0, GetWide(), GetTall(), Color( 90, 30, 30, 255 ), 1.0f, false, Color( 0, 0, 0, 0 ) ); // Resurgence

	int nBorder = MAX( YRES( 1 ), 1 );
	if ( IsArmed() || IsDepressed() )
	{
		DrawRoundedBox( nBorder, nBorder, GetWide() - nBorder * 2, GetTall() - nBorder * 2, Color( 113, 0, 0, 255 ), 1.0f, true, Color( 255, 28, 28, 255 ) ); // atrblizzard: changed colors to dark red
	}
	else if ( IsEnabled() )
	{
		DrawRoundedBox( nBorder, nBorder, GetWide() - nBorder * 2, GetTall() - nBorder * 2, Color( 64, 26, 26, 255 ), 1.0f, false, Color( 0, 0, 0, 0 ) ); // atrblizzard: changed colors to dark red
	}
	else
	{
		DrawRoundedBox( nBorder, nBorder, GetWide() - nBorder * 2, GetTall() - nBorder * 2, Color( 80, 52, 52, 255 ), 1.0f, false, Color( 0, 0, 0, 0 ) ); // atrblizzard: changed colors to dark red
	}
}
...
void CNB_Button::OnCursorEntered()
{
	if ( IsPC() )
	{
		...
		{
			vgui::surface()->PlaySound( "interface/mm_cursor_over.wav" );
		}
	}
