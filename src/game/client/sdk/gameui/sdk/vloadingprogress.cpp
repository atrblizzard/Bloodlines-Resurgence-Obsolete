...
void LoadingProgress::ApplySchemeSettings( IScheme *pScheme )
{
	...
	const char *pImageName = "interface/tipinfoscreen/loadingbar_full";
	...
	// find or create pattern
	// purposely not freeing these, not worth the i/o hitch for something so small
	pImageName = "interface/tipinfoscreen/loadingbar_empty";
...
void LoadingProgress::SetupControlStates()
{
	...
	if ( m_pBGImage )
	{
		...
		else
		{
			...
			V_snprintf( filename, sizeof( filename ), "console/console_backgroung" ); // TODO: engine->GetStartupImage( filename, sizeof( filename ), screenWide, screenTall );
...
void LoadingProgress::SetupPoster( void )
{
	...
	{ 
		...
		{
			case 1: pszPosterImage = ( m_bFullscreenPoster && bIsWidescreen ) ? "../interface/tipinfoscreen/background_04" : "../interface/tipinfoscreen/background_04"; break;
			case 2: pszPosterImage = ( m_bFullscreenPoster && bIsWidescreen ) ? "../interface/tipinfoscreen/background_03" : "../interface/tipinfoscreen/background_03"; break;
			case 3: pszPosterImage = ( m_bFullscreenPoster && bIsWidescreen ) ? "../interface/tipinfoscreen/background_02" : "../interface/tipinfoscreen/background_02"; break;
			case 4:
			default: pszPosterImage = ( m_bFullscreenPoster && bIsWidescreen ) ? "../interface/tipinfoscreen/background_default" : "../interface/tipinfoscreen/background_default"; break;
		}

		// if the image was cached this will just hook it up, otherwise it will load it
		pPoster->SetImage( pszPosterImage );
		DevMsg("Using widescreen scheme scheme file: %s\n", pszPosterImage);
		if ( pPoster->GetImage() )
		{
			bNamesVisible = true;
		}
	...
	SetControlEnabled( "LoadingTipPanel", true ); //changed
}
